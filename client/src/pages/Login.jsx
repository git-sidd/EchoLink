...
const Login = () => {
  ...
  const { mutate, isPending, error, isSuccess } = useMutation({
    mutationFn: async () => {
      const response = await axiosInstance.post("/auth/login", loginState);
      ...
      <div className="h-screen flex flex-col md:flex-row items-center md:justify-center justify-center-safe md:p-12 overflow-hidden " data-theme="forest">
        ...
        <div className="w-full md:w-1/2 space-y-6 md:px-10 p-4 ">
          <div className="text-center md:text-left">
            <h1 className="text-5xl  mt-2 font-extrabold mb-2 text-transparent bg-clip-text bg-gradient-to-r  from-pink-700 via-pink-400 to-pink-800 drop-shadow-lg tracking-wide">
              Echo Link
            </h1>
          ...
          <form onSubmit={handleSubmit} className="space-y-4 md:w-[90%]">
            ...
            <button
              type="submit"
              className="btn btn-secondary w-full mt-4 rounded-3xl"
            > {!isPending ? "Login" : <span className="loading loading-spinner"><p>Logging in...</p></span>}
            </button>
        ...
        <div className="hidden w-full md:w-1/2 md:gap-2 md:flex md:flex-col  md:px-10  justify-center">
          ...
          <p className="text-center">
            practice conversation,make friends, and improve your language skills
            together
          </p>
        </div>
      </div>
